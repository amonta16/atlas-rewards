  ...
  return (
    <header
      className={cn(
        ...
        scrolled || open ? "border-b border-white/[0.07] bg-[#07090f]/80 backdrop-blur-xl" : "border-b border-transparent",
      )}
    >
      ...
      <div className="lp-container flex h-16 items-center justify-between gap-6 md:h-[72px]">
        ...
        <nav className="hidden items-center gap-1 lg:flex" aria-label="Primary">
          {LINKS.map((l) => (
            <a key={l.href} href={l.href} className="lp-focus rounded-md px-3 py-2 text-[14px] font-medium text-zinc-300 transition-colors hover:text-white">
              {l.label}
            </a>
          ))}
        </nav>
        ...
        <div className="hidden items-center gap-2 lg:flex">
          <Link href="/login" className="lp-focus rounded-md px-3 py-2 text-[14px] font-medium text-zinc-300 hover:text-white">
            Business login
          </Link>
        ...
        <button
          type="button"
          className="lp-focus grid h-10 w-10 place-items-center rounded-lg text-white lg:hidden"
          aria-expanded={open}
      ...
      <div
        id="mobile-nav"
        className={cn(
          "lg:hidden overflow-hidden border-t border-white/[0.06] bg-[#07090f]/95 backdrop-blur-xl transition-[max-height,opacity] duration-300",
      ...
      >
        <nav className="lp-container flex flex-col gap-1 py-4" aria-label="Mobile">
          {LINKS.map((l) => (
            <a key={l.href} href={l.href} onClick={() => setOpen(false)} className="lp-focus rounded-lg px-3 py-3 text-base font-medium text-zinc-200 hover:bg-white/5">
              {l.label}
            </a>
          ))}
          <Link href="/login" onClick={() => setOpen(false)} className="lp-focus rounded-lg px-3 py-3 text-base font-medium text-zinc-200 hover:bg-white/5">
            Business login
          </Link>
