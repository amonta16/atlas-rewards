    ...
    ui: (
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <span className="ml-auto text-xs text-zinc-500">Brand colors</span>
        </div>
    ...
    <section id={ANCHORS.howItWorks} className="relative scroll-mt-24 py-20 md:py-28" aria-labelledby="hiw-title">
      ...
      <div className="lp-container">
        <Reveal className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-lg text-zinc-400">You run the business. We run the software.</p>
        </Reveal>
          ...
          {STEPS.map((s, i) => (
            <Reveal as="li" key={s.n} delay={i * 90} className="lp-card group relative flex flex-col p-6">
              <div className="flex items-center justify-between">
                <span className="font-mono text-sm text-cyan-300">{s.n}</span>
                <span className="grid h-9 w-9 place-items-center rounded-lg border border-white/10 bg-white/[0.04] text-zinc-200 transition-colors group-hover:border-cyan-300/40 group-hover:text-cyan-200">
              ...
              <div className="mt-5 rounded-xl border border-white/[0.07] bg-[#070a10] p-3.5 text-xs">{s.ui}</div>
            </Reveal>
          ))}
        </ol>
        ...
        <Reveal className="mt-10 flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-sm text-zinc-500">Typical launch: days, not months. <span className="lp-placeholder whitespace-nowrap rounded px-1.5 py-0.5 font-mono text-[10px]">[ CONFIRM TYPICAL LAUNCH TIME ]</span></p>
        </Reveal>
      </div>
    </section>
  );
...
function Row({ label, value, mono, ok }: { label: string; value: string; mono?: boolean; ok?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <span className={`${mono ? "font-mono" : ""} ${ok ? "text-emerald-300" : "text-zinc-200"} font-medium`}>{value}</span>
    </div>
  );
}
