...
/** Footer — CP-145. Logo + one line, three short link columns. */
export function Footer() {
  const year = new Date().getFullYear();
  return (
    <footer className="border-t border-[#e3e9f0] py-12" aria-label="Footer">
      <div className="lp-container grid gap-10 md:grid-cols-[1.4fr_1fr_1fr_1fr]">
        <div>
          ...
          <p className="mt-4 max-w-xs text-sm leading-relaxed text-slate-500">Branded rewards apps for local businesses.</p>
          <a href={`mailto:${CONTACT_EMAIL}`} className="lp-focus mt-3 inline-block rounded text-sm text-slate-700 hover:text-[#14213d]">
            {CONTACT_EMAIL}
          </a>
        </div>
        <Col
        ...
        <Col
          title="Company"
          links={[
            { href: "/login", label: "Business log in" },
            { href: "/legal/terms", label: "Terms" },
            { href: "/legal/privacy", label: "Privacy" },
          ]}
        />
      </div>
      <div className="lp-container mt-10 flex flex-col gap-2 border-t border-[#e3e9f0] pt-6 text-xs text-slate-400 sm:flex-row sm:items-center sm:justify-between">
