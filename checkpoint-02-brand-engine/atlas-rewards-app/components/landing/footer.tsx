...
export function Footer() {
  ...
  return (
    <footer className="border-t border-white/[0.07] py-12" aria-label="Footer">
      <div className="lp-container grid gap-10 md:grid-cols-[1.4fr_1fr_1fr_1fr]">
        <div>
          ...
          <a href={`mailto:${CONTACT_EMAIL}`} className="lp-focus mt-4 inline-block rounded text-sm text-zinc-300 hover:text-white">
            {CONTACT_EMAIL}
          </a>
        </div>
      ...
      <div className="lp-container mt-10 flex flex-col gap-2 border-t border-white/[0.06] pt-6 text-xs text-zinc-600 sm:flex-row sm:items-center sm:justify-between">
    ...
    <div>
      ...
      <ul className="mt-4 space-y-2.5">
        {links.map(({ href, label, ext }) => (
          <li key={href}>
            {ext ? (
              <a href={href} target="_blank" rel="noopener" className="lp-focus rounded text-sm text-zinc-300 hover:text-white">
                {label}
              </a>
            ) : (
              <Link href={href} className="lp-focus rounded text-sm text-zinc-300 hover:text-white">
                {label}
              </Link>
            )}
          </li>
