      ...
      <div className="grid grid-cols-2 gap-1 rounded-2xl border border-[#e8dfd1] bg-[#fbf8f2] p-3 md:block md:p-5">
        ...
        <div className="col-span-2 mt-1 border-t border-[#e8dfd1] pt-3 md:mt-3">
          <label className="flex items-center gap-2 text-xs text-slate-500">
            ...
            <select value={tz} onChange={(e) => setTz(e.target.value)} className="lp-focus h-8 flex-1 rounded-md border border-[#e8dfd1] bg-white px-2 text-xs text-slate-700">
              {tzOptions.map((z) => (
        ...
        {step === "date" && (
          <div>
            <div className="flex items-center justify-between">
              <button type="button" onClick={() => setView((v) => (v.m === 0 ? { y: v.y - 1, m: 11 } : { y: v.y, m: v.m - 1 }))} disabled={!canPrev} className="lp-focus grid h-9 w-9 place-items-center rounded-lg border border-[#e8dfd1] bg-white text-slate-600 disabled:opacity-30" aria-label="Previous month">
                <ChevronLeft className="h-4 w-4" />
              </button>
              <div className="text-sm font-semibold text-[#14213d]" aria-live="polite">{monthLabel}</div>
              <button type="button" onClick={() => setView((v) => (v.m === 11 ? { y: v.y + 1, m: 0 } : { y: v.y, m: v.m + 1 }))} className="lp-focus grid h-9 w-9 place-items-center rounded-lg border border-[#e8dfd1] bg-white text-slate-600" aria-label="Next month">
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isToday && !selected && "ring-1 ring-[#e07a3f]",
                    )}
                  >
                    {d}
                  </button>
