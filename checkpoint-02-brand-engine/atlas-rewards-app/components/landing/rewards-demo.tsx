          ...
          <Reveal delay={240} className="lp-card flex flex-col p-6 sm:p-8">
            ...
            <ul className="mt-6 grid gap-2.5 text-[15px]">
              {[
                ...
                <li key={t} className="flex items-start gap-2.5 text-zinc-300">
                  <span className="mt-[3px] grid h-[18px] w-[18px] shrink-0 place-items-center rounded-full bg-cyan-400/15 text-cyan-300">
  ...
  return (
    <div ref={ref} className="mt-5">
      <div className="flex items-center gap-4">
        <div className={cn("relative grid h-16 w-16 place-items-center rounded-2xl bg-gradient-to-br from-orange-400 to-rose-500 text-white shadow-[0_10px_30px_-10px_rgba(251,146,60,0.8)]", celebrate && "lp-pulse-once")}>
          <Flame className="h-8 w-8" aria-hidden />
        </div>
        <div>
          <div className="flex items-baseline gap-2">
            <span key={days} className="lp-pop text-4xl font-bold tabular-nums text-white">
              {days}
            </span>
            <span className="text-zinc-400">day streak</span>
          </div>
          <div className={cn("mt-1 flex items-center gap-1.5 text-sm transition-colors", celebrate ? "text-emerald-300" : "text-zinc-500")} role="status">
            {celebrate ? (
              <>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                done ? "border-orange-300/40 bg-orange-400/20 text-orange-200" : "border-white/[0.07] bg-white/[0.02] text-zinc-600",
                milestone && done && "lp-pop border-amber-300/60 bg-amber-300/25 text-amber-100",
              )}
      ...
      <button type="button" onClick={bump} className="lp-focus mt-4 text-xs text-zinc-500 underline-offset-2 hover:text-zinc-300 hover:underline">
  ...
  return (
    <div ref={ref} className="mt-5">
      <div className="text-3xl font-bold tabular-nums text-white">
        <AnimatedCounter from={740} to={go ? 1000 : 740} duration={1600} start={go} /> <span className="whitespace-nowrap text-base font-medium text-zinc-500">/ 1,000 pts</span>
      </div>
      <div className={cn("mt-2 flex items-center gap-1.5 text-sm transition-colors", go ? "text-emerald-300" : "text-zinc-500")} role="status" aria-live="polite">
        {go ? (
          <>
      ...
      <div className="mt-3 h-3 overflow-hidden rounded-full bg-white/[0.06]">
        <div
          className={cn("h-full rounded-full bg-gradient-to-r from-cyan-400 to-blue-500", go && "lp-shimmer")}
