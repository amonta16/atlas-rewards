...
export function InteractiveDemo() {
  ...
  return (
    <section id={ANCHORS.demo} className="relative scroll-mt-24 py-20 md:py-28" aria-labelledby="demo-title">
      <div className="pointer-events-none absolute inset-x-0 top-1/2 h-[600px] -translate-y-1/2 bg-[radial-gradient(50%_50%_at_70%_50%,rgba(96,165,250,0.12),transparent)]" aria-hidden />
      <div className="lp-container grid items-center gap-12 lg:grid-cols-[1fr_auto] lg:gap-20">
        <Reveal>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active
                      ? "border-white/30 bg-white text-[#06101a] shadow-[0_8px_30px_-10px_rgba(255,255,255,0.5)]"
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center">
            ...
            <p className="text-sm text-zinc-500">Bring a logo to the demo and we&apos;ll mock yours up live.</p>
          </div>
        </Reveal>
      ...
      <dd className="mt-1 flex items-center gap-2 text-sm font-medium text-white">
        {swatch && (
          <span className="flex -space-x-1" aria-hidden>
            {swatch.map((c) => (
              <span key={c} className="h-4 w-4 rounded-full ring-2 ring-[#0b1017] transition-colors duration-500" style={{ background: c }} />
            ))}
          </span>
        )}
