...
const field =
  "lp-focus h-11 w-full rounded-lg border border-white/10 bg-white/[0.04] px-3.5 text-[15px] text-white placeholder:text-zinc-500 focus:border-cyan-400/50";
    ...
    return (
      <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/10 p-6 text-center">
        ...
        <p className="mt-1 text-sm text-zinc-300">
          ...
          <a className="text-cyan-300 underline-offset-2 hover:underline" href={`mailto:${CONTACT_EMAIL}`}>
            {CONTACT_EMAIL}
          </a>
        </p>
        {onDone && (
          <button type="button" onClick={onDone} className="lp-focus mt-5 h-10 rounded-lg bg-white px-5 text-sm font-semibold text-[#06101a]">
            Done
          </button>
        )}
      </div>
    ...
    <form onSubmit={submit} className={cn("grid gap-4", compact ? "sm:grid-cols-2" : "sm:grid-cols-2")} noValidate={false}>
      ...
      <label className="grid gap-1.5 text-sm">
        ...
        <select name="industry" className={cn(field, "appearance-none")} defaultValue="">
          ...
          {INDUSTRIES.map((i) => (
            <option key={i} value={i} className="bg-[#0b1017]">
              {i}
            </option>
          ))}
        </select>
      ...
      <label className="grid gap-1.5 text-sm">
        <span className="text-zinc-300">Best time to call</span>
        <select name="preferred_time" className={cn(field, "appearance-none")} defaultValue="Any time">
          {TIMES.map((t) => (
            <option key={t} value={t} className="bg-[#0b1017]">
              {t}
            </option>
          ))}
        </select>
      ...
      <div className="sm:col-span-2 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-xs text-zinc-500">No contracts to sign here — this just books a conversation.</p>
        <button
          type="submit"
          disabled={state === "sending"}
          className="lp-focus lp-cta-primary inline-flex h-12 shrink-0 items-center justify-center gap-2 whitespace-nowrap rounded-xl bg-white px-6 font-semibold text-[#06101a] hover:bg-cyan-50 disabled:opacity-60"
        >
