      ...
      <div className="lp-container">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-6">
          ...
          <Reveal delay={80} className="lp-card p-6 sm:p-8 md:col-span-2">
            ...
            <div className="mt-6 space-y-2.5">
              ...
              <div className="flex justify-end">
                <span className="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-2 text-xs font-semibold text-[#06101a]">
                  <Star className="h-3.5 w-3.5 fill-amber-400 text-amber-400" aria-hidden /> Leave a review
                </span>
              </div>
              <Bubble side="biz" muted>
                Review posted · <b className="text-emerald-300">+50 pts</b> added
              </Bubble>
            </div>
          </Reveal>
          ...
          <Reveal className="lp-card p-6 sm:p-8 md:col-span-2">
            ...
            <p className="mt-5 text-xs text-zinc-500">Rewards release only after the friend&apos;s qualifying visit — no freebies for fake signups.</p>
          </Reveal>
          ...
          <Reveal delay={80} className="lp-card p-6 sm:p-8 md:col-span-2">
            ...
            <div className="mt-6 rounded-xl border border-pink-400/20 bg-gradient-to-br from-pink-500/10 to-violet-500/10 p-4">
              <div className="flex items-center gap-3">
                <span className="grid h-10 w-10 place-items-center rounded-lg bg-pink-400/20 text-pink-200">
                  <Cake className="h-5 w-5" aria-hidden />
                </span>
                <div>
                  <div className="text-sm font-semibold text-white">Happy birthday, Jordan 🎂</div>
                  <div className="text-xs text-zinc-400">A free dessert is waiting in your app this week.</div>
                </div>
              </div>
          ...
          <Reveal delay={120} className="lp-card p-6 sm:p-8 md:col-span-2">
            ...
            <div className="mt-6 grid grid-cols-[auto_1fr] items-center gap-4">
              <div className="grid h-20 w-20 place-items-center rounded-xl bg-white p-2">
                <QrCode className="h-full w-full text-[#06101a]" aria-hidden />
              </div>
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-sm text-zinc-300">
                  <KeyRound className="h-4 w-4 text-cyan-300" aria-hidden /> Staff PIN login
                </div>
                <div className="grid grid-cols-3 gap-1" aria-hidden>
                  {["1", "2", "3", "4", "5", "6"].map((k) => (
                    <span key={k} className="grid h-7 place-items-center rounded-md bg-white/[0.06] text-xs text-zinc-300">
                      {k}
                    </span>
                  ))}
                </div>
                <div className="text-xs text-zinc-500">Award points · redeem · undo in 30s</div>
              </div>
            </div>
          </Reveal>
        </div>
...
function Head({ icon: I, title, why }: { icon: React.ComponentType<{ className?: string }>; title: string; why: string }) {
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-zinc-400">{why}</p>
    </div>
  );
}
...
function Bubble({ children, side, muted }: { children: React.ReactNode; side: "biz"; muted?: boolean }) {
  return (
    <div className={cn("max-w-[92%] rounded-xl rounded-tl-sm px-3.5 py-2.5 text-[13px] leading-snug", muted ? "bg-white/[0.03] text-zinc-400" : "bg-white/[0.07] text-zinc-200")} data-side={side}>
      {children}
    </div>
  );
}
...
function Node({ label, sub, icon: I, accent }: { label: string; sub: string; icon: React.ComponentType<{ className?: string }>; accent?: boolean }) {
  return (
    <div className="flex flex-col items-center text-center">
      <span className={cn("grid h-11 w-11 place-items-center rounded-xl border", accent ? "border-cyan-300/40 bg-cyan-400/15 text-cyan-200" : "border-white/10 bg-white/[0.04] text-zinc-200")}>
...
function Connector() {
  return <span className="mb-7 h-px flex-1 bg-gradient-to-r from-white/10 via-cyan-300/50 to-white/10" aria-hidden />;
}
    ...
    <div ref={ref} className="mt-6 grid gap-3 sm:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] sm:items-stretch">
      {steps.map((s, i) => (
        <div key={s.l} className="contents">
          <div
            className={cn("rounded-xl border border-white/[0.08] bg-white/[0.03] p-4 transition-all duration-500", inView ? "translate-y-0 opacity-100" : "translate-y-3 opacity-0")}
            style={{ transitionDelay: `${i * 220}ms` }}
          >
            <s.icon className="h-4 w-4 text-cyan-300" aria-hidden />
            <div className="mt-2 text-2xl font-semibold tabular-nums text-white">{s.n}</div>
            <div className="mt-1 text-xs leading-snug text-zinc-400">{s.l}</div>
          </div>
          {i < steps.length - 1 && (
            <div className="grid place-items-center text-zinc-600 transition-opacity duration-500" style={{ transitionDelay: `${i * 220 + 110}ms`, opacity: inView ? 1 : 0 }} aria-hidden>
