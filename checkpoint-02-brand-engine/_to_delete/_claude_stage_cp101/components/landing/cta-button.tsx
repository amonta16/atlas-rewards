...
export function DemoCta({
  ...
  const cls = cn(
    base,
    sizes[size],
    "lp-cta-primary bg-[#1f5f8b] text-white hover:bg-[#174a6e] active:translate-y-px",
    className,
  );
  const onClick = () => {
    track(event, { source });
  ...
  return (
    <button
      ...
      className={cn(
        base,
        sizes[size],
        "text-[#14213d] border border-[#e8dfd1] bg-white hover:bg-[#f3ede2] hover:border-[#e8dfd1] backdrop-blur-sm",
        className,
      )}
      onClick={() => {
    ...
    >
      <span className="grid h-6 w-6 place-items-center rounded-full bg-[#e07a3f]/15 text-[#e07a3f]">
