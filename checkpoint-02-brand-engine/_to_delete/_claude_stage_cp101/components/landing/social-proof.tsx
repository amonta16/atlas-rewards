        ...
        <dl className="mt-12 grid grid-cols-2 gap-4 lg:grid-cols-4">
          {STATS.map((s, i) => (
            <Reveal key={s.l} delay={i * 70} className="lp-card p-5 text-center sm:p-6">
              ...
              <dt className="mt-2 text-sm text-slate-700">{s.l}</dt>
              <div className="mt-2 font-mono text-[10px] text-slate-400">{s.note}</div>
            </Reveal>
          ))}
        </dl>
        ...
        <div className="mt-12 grid gap-4 lg:grid-cols-3">
          {TESTIMONIALS.map((t, i) => (
            <Reveal key={t.slot} delay={i * 90} as="article" className="lp-card flex flex-col p-6 sm:p-7">
              <Quote className="h-6 w-6 text-[#1f5f8b]/70" aria-hidden />
              <blockquote className="mt-4 flex-1 text-[15px] leading-relaxed text-slate-800">{t.q}</blockquote>
              <footer className="mt-6 flex items-center gap-3">
                <div className="lp-placeholder grid h-11 w-11 shrink-0 place-items-center rounded-full text-[8px] text-slate-500">[ HEADSHOT ]</div>
                <div>
                  <div className="text-sm font-semibold text-[#14213d]">{t.n}</div>
                  <div className="text-xs text-slate-500">{t.c}</div>
                </div>
                <span className="ml-auto font-mono text-[10px] text-slate-400">[ TESTIMONIAL {t.slot} ]</span>
              </footer>
            </Reveal>
          ))}
        </div>
