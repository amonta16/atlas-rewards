...
export function VideoPlayer({ autoStart = false }: { autoStart?: boolean }) {
  ...
  const frame = "relative aspect-video w-full overflow-hidden rounded-2xl border border-[#e8dfd1] bg-white shadow-[0_30px_80px_-30px_rgba(20,33,61,0.35)]";
  ...
  return (
    <div className={frame} role="img" aria-label="Atlas demo video placeholder">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 p-6 text-center">
        <button type="button" onClick={start} className="lp-focus group grid h-20 w-20 place-items-center rounded-full bg-white text-[#14213d] shadow-[0_0_0_12px_rgba(255,255,255,0.35)] transition-transform hover:scale-105" aria-label="Play demo video">
...
function PosterOverlay({ onPlay }: { onPlay: () => void }) {
  return (
    ...
    >
      <span className="grid h-20 w-20 place-items-center rounded-full bg-white text-[#14213d] shadow-[0_0_0_12px_rgba(255,255,255,0.35)] transition-transform group-hover:scale-105">
        <Play className="ml-1 h-8 w-8 fill-current" aria-hidden />
      </span>
      <span className="absolute bottom-4 right-4 rounded-md bg-white/90 px-2 py-1 font-mono text-xs text-slate-700">{VSL.durationLabel}</span>
    </button>
