  ...
  return (
    <header
      className={cn(
        ...
        scrolled || open ? "border-b border-[#e8dfd1] bg-[#fbf8f2]/90 backdrop-blur-md" : "border-b border-transparent",
      )}
    >
      ...
      <div className="lp-container flex h-16 items-center justify-between gap-6 md:h-[72px]">
        <Link href="/" className="lp-focus flex items-center rounded-md" aria-label="Atlas Engine home">
          ...
          <img src="/landing/atlas-engine-logo-navy.png" alt="Atlas Engine" width={1315} height={494} className="h-8 w-auto md:h-9" />
        </Link>
      ...
      <div
        id="mobile-nav"
        className={cn(
          "lg:hidden overflow-hidden border-t border-[#e8dfd1] bg-[#fbf8f2]/90 backdrop-blur-md transition-[max-height,opacity] duration-300",
      ...
      >
        <nav className="lp-container flex flex-col gap-1 py-4" aria-label="Mobile">
          {LINKS.map((l) => (
            <a key={l.href} href={l.href} onClick={() => setOpen(false)} className="lp-focus rounded-lg px-3 py-3 text-base font-medium text-slate-800 hover:bg-[#f3ede2]">
              {l.label}
            </a>
          ))}
          <Link href="/login" onClick={() => setOpen(false)} className="lp-focus rounded-lg px-3 py-3 text-base font-medium text-slate-800 hover:bg-[#f3ede2]">
            Business login
          </Link>
