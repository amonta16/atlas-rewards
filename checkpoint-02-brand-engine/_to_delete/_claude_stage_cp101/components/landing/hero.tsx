...
export function Hero() {
  ...
  return (
    <section className="relative overflow-hidden pt-28 md:pt-36 lg:pt-40" aria-labelledby="hero-title">
      {/* Atmosphere */}
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-[#e9f2f8] via-[#f8f4ec] to-[#fbf8f2]" aria-hidden />
      <div className="lp-drift pointer-events-none absolute -top-32 right-[-10%] h-[520px] w-[520px] rounded-full bg-[radial-gradient(closest-side,rgba(240,163,94,0.35),transparent)] blur-3xl" aria-hidden />
      <div className="pointer-events-none absolute left-[-10%] top-24 h-[460px] w-[460px] rounded-full bg-[radial-gradient(closest-side,rgba(42,143,181,0.22),transparent)] blur-3xl" aria-hidden />
      <div className="lp-noise pointer-events-none absolute inset-0" aria-hidden />

      <div className="lp-container relative grid items-center gap-14 lg:grid-cols-[1.1fr_0.9fr] lg:gap-10">
        <div className="max-w-2xl">
          <p className="lp-eyebrow">
            <Zap className="h-3.5 w-3.5 text-[#e07a3f]" aria-hidden />
        ...
        <div className="relative mx-auto w-full max-w-[420px] lg:max-w-none" id={ANCHORS.product}>
          <div className="pointer-events-none absolute left-1/2 top-1/2 h-[420px] w-[420px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[radial-gradient(closest-side,rgba(224,122,63,0.18),transparent)] blur-xl" aria-hidden />
          ...
          <FloatingChip className="right-0 top-[44%] lp-float-slower hidden sm:flex lg:-right-6 xl:-right-12" icon={<Flame className="h-4 w-4 text-orange-600" />}>
            <b className="text-[#14213d]">Streak: 5 days</b> <span className="text-slate-600">reward unlocked</span>
          </FloatingChip>
          <FloatingChip className="left-[2%] bottom-[16%] lp-float hidden sm:flex lg:-left-8 xl:-left-14" icon={<Star className="h-4 w-4 text-amber-700" />}>
...
function FloatingChip({ children, className, icon }: { children: React.ReactNode; className?: string; icon: React.ReactNode }) {
  return (
    <div className={`absolute z-10 items-center gap-2 rounded-xl border border-[#e8dfd1] bg-white/95 px-3 py-2 text-[13px] shadow-[0_12px_30px_-10px_rgba(20,33,61,0.25)] backdrop-blur ${className ?? ""}`}>
