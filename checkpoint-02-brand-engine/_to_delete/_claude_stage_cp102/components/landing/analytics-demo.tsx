      ...
      <div className="lp-container grid gap-12 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
        <Reveal>
          ...
          <ul className="mt-6 space-y-2 text-[15px] text-slate-700">
            <li className="flex gap-2"><ArrowUpRight className="mt-1 h-4 w-4 text-[#1f5f8b]" aria-hidden /> Revenue driven vs. your pre-Atlas baseline</li>
            <li className="flex gap-2"><ArrowUpRight className="mt-1 h-4 w-4 text-[#1f5f8b]" aria-hidden /> Inactive customers detected and won back</li>
            <li className="flex gap-2"><ArrowUpRight className="mt-1 h-4 w-4 text-[#1f5f8b]" aria-hidden /> Google review funnel — requested, clicked, posted</li>
          </ul>
          <div className="mt-8">
        ...
        <Reveal delay={100}>
          <div ref={ref} className="lp-card relative overflow-hidden p-5 sm:p-6">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm font-semibold text-[#14213d]">Casa Verde · Impact</div>
                <div className="text-xs text-slate-500">Last 30 days</div>
              </div>
              <span className="lp-placeholder rounded-md px-2 py-1 font-mono text-[10px] text-slate-600">[ DEMO METRICS — REPLACE WITH REAL DATA ]</span>
            </div>
            ...
            {/* Revenue chart */}
            <div className="lp-light mt-4 rounded-xl border border-[#e8dfd1] bg-white p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm text-slate-700">
                  <DollarSign className="h-4 w-4 text-emerald-700" aria-hidden /> Revenue driven by Atlas
                </div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs">
              {[
                ...
                <div key={l} className="lp-light rounded-lg border border-[#e8dfd1] bg-white py-2.5">
                  <div className="text-base font-semibold text-[#14213d]">{n}</div>
...
function Stat({ icon: I, label, value, delta }: { icon: React.ComponentType<{ className?: string }>; label: string; value: React.ReactNode; delta: string }) {
  return (
    <div className="lp-light rounded-xl border border-[#e8dfd1] bg-white p-3.5">
      ...
      <div className="mt-1.5 text-2xl font-semibold tabular-nums text-[#14213d]">{value}</div>
      <div className="mt-0.5 text-xs text-emerald-700">{delta}</div>
    </div>
  );
}
...
function Bars({ active }: { active: boolean }) {
  ...
  return (
    <div className="mt-4 flex h-24 items-end gap-1.5" aria-hidden>
      {data.map((h, i) => (
        <div key={i} className="flex-1 rounded-t-sm bg-gradient-to-t from-[#1f5f8b]/50 to-[#2a8fb5]" style={{ height: active ? `${h}%` : "4%", transition: `height 900ms cubic-bezier(0.22,1,0.36,1) ${i * 60}ms` }} />
      ))}
    </div>
  );
}
