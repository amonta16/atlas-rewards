...
// CP-112: live-client manager + operating costs
import Link from "next/link";
import { Wallet, Server } from "lucide-react";
import type { RecurringBill, ExpenseCategory } from "@/lib/types/database";
import { SubsManagerButton } from "@/components/agency/bookkeeping/subs-manager";
import { computeSnapshot } from "@/lib/bookkeeping";
...
type ExpenseMonth = {
  month_start: string; hosting_cents: number; recurring_cents: number;
  onetime_cents: number; total_cents: number;
};
...
export function AnalyticsClient({
  ...
  expenseMonthly, bkBills, bkCategories,
}: {
  friendlyName: string;
  todayIso: string;
  ...
  expenseMonthly: ExpenseMonth[];
  bkBills: RecurringBill[];
  bkCategories: ExpenseCategory[];
}) {
  ...
  // CP-112: billing figures are live state so the Manage-live-clients modal
  // and the Log-MRR form update the page immediately after a save.
  const [billingLive, setBillingLive] = useState<BillingSummary | null>(billing);
  const [mrrBiz, setMrrBiz] = useState(mrrByBusiness);
  const liveMrrCents = Number(billingLive?.mrr_cents ?? 0);
  const activeClients = Number(billingLive?.active_subscriptions ?? 0);

  async function refreshBilling() {
    const supabase = createClient();
    const [{ data: sum }, { data: biz }] = await Promise.all([
      supabase.rpc("agency_billing_summary"),
      supabase.rpc("agency_mrr_by_business"),
    ]);
    if (sum) setBillingLive((Array.isArray(sum) ? sum[0] : sum) as BillingSummary);
    if (biz) setMrrBiz(biz as typeof mrrByBusiness);
    syncSnapshot();
  }
  ...
  const rankBiz = useMemo(() => mrrBiz.map(b => ({
  ...
  return (
    <div className="min-h-screen" style={{ background: HQ.canvas }}>
      ...
      <header className="relative px-4 sm:px-8 pt-10 pb-6 overflow-hidden">
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <SubsManagerButton businesses={businesses} onChanged={refreshBilling} />
            <ManualBillingButton onSaved={refreshBilling} />
      ...
      <div className="px-4 sm:px-8 pb-12 space-y-5">
        ...
        {/* CP-112: Operating Costs & Margin — verified bookkeeping data */}
        <OperatingCosts
          liveMrrCents={liveMrrCents}
          liveDaily={initialLiveDaily}
...
/* ──────────────── CP-112: Operating Costs & Margin ──────────────── */

function OperatingCosts({
  liveMrrCents, liveDaily, expenseMonthly, bills, categories, todayIso,
}: {
  liveMrrCents: number;
  liveDaily: LiveDay[];
  expenseMonthly: ExpenseMonth[];
  ...
  // This month's paid figures come straight from the bookkeeping ledger.
  const thisMonth = expenseMonthly.find(m => m.month_start.slice(0, 7) === todayIso.slice(0, 7));
  const snap = useMemo(
    () => computeSnapshot(bills, [], categories, liveMrrCents, todayIso),
    [bills, categories, liveMrrCents, todayIso]);
  const hostingPaid = thisMonth?.hosting_cents ?? 0;
  const otherRecurring = thisMonth?.recurring_cents ?? 0;
  const onetime = thisMonth?.onetime_cents ?? 0;
  const totalPaid = thisMonth?.total_cents ?? 0;
  const remainder = liveMrrCents - totalPaid;
  const hostingPct = liveMrrCents > 0 ? Math.round((hostingPaid / liveMrrCents) * 100) : null;
  const marginPct = liveMrrCents > 0 ? Math.round(((liveMrrCents - hostingPaid) / liveMrrCents) * 100) : null;

  // Month-end live MRR from the real daily reconstruction, matched to the
  // months the expense rollup covers.
  const mrrByMonth = useMemo(() => {
    const map = new Map<string, LiveDay>();
    for (const d of liveDaily) {
      const key = d.day.slice(0, 7);
      const prev = map.get(key);
      if (!prev || d.day > prev.day) map.set(key, d);
    }
    return map;
  }, [liveDaily]);

  const costSeries: ChartSeries[] = useMemo(() => {
    const months = expenseMonthly.map(m => m.month_start);
    const mrrPts: { date: string; value: number }[] = [];
    for (const m of months) {
      const hit = mrrByMonth.get(m.slice(0, 7));
      if (hit) mrrPts.push({ date: m, value: hit.mrr_cents });
    }
    const hasAnyExpense = expenseMonthly.some(m => m.total_cents > 0);
    return [
      { key: "mrr", label: "Live MRR (month-end)", color: "#34d399", points: mrrPts },
      {
        key: "hosting", label: "Hosting paid", color: "#38bdf8", dashed: true,
        points: hasAnyExpense ? expenseMonthly.map(m => ({ date: m.month_start, value: m.hosting_cents })) : [],
      },
      {
        key: "total", label: "Total expenses paid", color: "#fbbf24",
        points: hasAnyExpense ? expenseMonthly.map(m => ({ date: m.month_start, value: m.total_cents })) : [],
      },
      {
        key: "remainder", label: "Est. remainder", color: "#a78bfa",
        points: hasAnyExpense
          ? expenseMonthly.map(m => {
              const hit = mrrByMonth.get(m.month_start.slice(0, 7));
              return { date: m.month_start, value: (hit?.mrr_cents ?? 0) - m.total_cents };
            })
          : [],
      },
    ];
  }, [expenseMonthly, mrrByMonth]);

  return (
    <GlassPanel
      title="Operating Costs & Margin"
      subtitle="From the Bookkeeping ledger — paid expenses only, monthly commitments labeled separately. Estimates, not accounting profit."
      icon={<Server className="h-4 w-4" />}
      right={
        <Link href="/agency/bookkeeping"
          ...
          <Wallet className="h-4 w-4" /> Open Bookkeeping
        </Link>
      }
    >
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-2.5 mb-4">
        <SummaryCard icon={<Server className="h-4 w-4" />} label="Hosting paid (mo)"
          value={dollars(hostingPaid)} sub={`${dollars(snap.hostingCommitmentCents)}/mo committed`} />
        <SummaryCard icon={<Wallet className="h-4 w-4" />} label="Other recurring (mo)"
          value={dollars(otherRecurring)} sub="Paid this month" />
        <SummaryCard icon={<Wallet className="h-4 w-4" />} label="One-time (mo)"
          value={dollars(onetime)} sub="Paid this month" />
        <SummaryCard icon={<Wallet className="h-4 w-4" />} label="Total paid (mo)"
          value={dollars(totalPaid)} sub={`${dollars(snap.monthlyCommitmentCents)}/mo committed`} />
        <SummaryCard icon={<Scale className="h-4 w-4" />} label="Est. remainder"
          value={liveMrrCents > 0 || totalPaid > 0 ? dollars(remainder) : "—"}
          sub="MRR − paid · estimate" accent="#a78bfa" />
        <SummaryCard icon={<TrendingUp className="h-4 w-4" />} label="Hosting % / margin"
          value={hostingPct != null ? `${hostingPct}% / ${marginPct}%` : "—"}
          sub="Of live MRR · estimate" warn={hostingPct != null && hostingPct > 50} />
      </div>

      <RevenueChart series={costSeries} formatValue={dollars} height={220} />

      {snap.upcomingBills.length > 0 && (
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <span className="text-[11px] font-bold uppercase tracking-widest text-sky-200/50">Upcoming commitments:</span>
          {snap.upcomingBills.slice(0, 5).map(b => (
            <Chip key={b.id} tone="amber">{b.vendor} · {dollars(b.amount_cents)} · {dateLabel(b.next_due_date)}</Chip>
          ))}
        </div>
      )}
    </GlassPanel>
