# CP-111 — Instant demo builder (Phase 1)
...
## Deploy
1. Apply `cp111_instant_demo.sql` in the Supabase SQL editor (creates the `create_demo_business` RPC — non-destructive, idempotent).
2. Deploy the app code.
3. Optional: run `cp111_seed_test.sql` (rolls itself back) → expect `✅ CP-111 DEMO SEED OK`.

## Files
- `cp111_instant_demo.sql` — the `create_demo_business(name, slug, industry, brand_colors, logo_url, pack)` RPC. Agency-staff gated; seeds everything atomically; slugs auto-suffix so batch runs never collide.
