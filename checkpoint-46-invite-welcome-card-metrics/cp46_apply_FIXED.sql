...
-- =====================================================================
-- FIXED COPY (cp46_apply_FIXED.sql). Run THIS whole file in the Supabase
-- SQL editor. It does NOT touch the points_ledger rule_type constraint,
-- so it cannot raise error 23514.
--
-- If you previously dropped the constraint, good — leave it dropped.
-- If not, this one-liner is safe to run first (it won't fail):
...
-- ---------------------------------------------------------------------
-- award_points() got a CP-44 auth gate that rejects positive awards unless
-- the caller staffs the business. During signup the caller IS the new
-- customer, so the trigger's award_points() call always raised 'permission
-- denied'. We now credit INLINE inside this SECURITY DEFINER trigger.
--
-- We deliberately do NOT touch the points_ledger rule_type CHECK — CP-44.1
-- dropped it on purpose (newer features write 'mystery_bonus' /
-- 'streak_milestone' / 'winback_bonus'). Re-adding any enum re-validates
-- history and fails (error 23514). 'signup_bonus' inserts cleanly.
...
--    trend graphs. Access-gated to staff of the business.
-- ---------------------------------------------------------------------
